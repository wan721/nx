...
int queue_ng_create( unsigned long qsize )
{
	...
	prints( "Elements: %p, elements[ 0 ]: %p, elements[ 1 ]: %p, elements[ 0 ].type: %p, elements[ 0 ].data %p\n",
			ngqueue->elements,
			&ngqueue->elements[ 0 ],
			&ngqueue->elements[ 1 ],
			&(ngqueue->elements[ 0 ].type),
			&(ngqueue->elements[ 0 ].data) );
...
void queue_ng_destroy( void )
{
	...
	prints("queueNGDestroy: freeCount: %d, justCount: %d, NGSyncQueue->head: %lu, NGSyncQueue->tail: %lu\n",
